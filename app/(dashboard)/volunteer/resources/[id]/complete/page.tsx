...
export default async function VolunteerCompleteForMemberPage({ params, searchParams }: PageProps) {
  ...
  // Get volunteer user from database
  ...
  if (!memberId || !/^[0-9a-fA-F]{24}$/.test(memberId)) {
    redirect(`/volunteer/resources/${resourceId}`);
  }

  // Fetch the member
  const member = await prisma.user.findUnique({
    where: { id: memberId },
    select: { id: true, firstName: true, lastName: true, email: true, familyId: true, role: true },
  });
  ...
  // VOLUNTEER: verify member is in an assigned family
  if (!member.familyId) {
    redirect(`/volunteer/resources/${resourceId}`);
  }

  const familyAssignment = await prisma.volunteerFamilyAssignment.findFirst({
    where: {
  ...
  // Fetch the MEMBER's existing form response (not the volunteer's)
